...
DatatableContentsStories.add('DatatableContents/interactive', () => {
  ...
  .add('DatatableContents/loading', () => <Components.DatatableContents {...defaultProps.DatatableContents} loading={true} />)
  .add('DatatableContents/error', () => <Components.DatatableContents {...defaultProps.DatatableContents} error={{ message: 'foo' }} />)
  ...
  .add('DatatableContents/withoutColumns', () => <Components.DatatableContents {...defaultProps.DatatableContentsWithoutColumns} />)

  //NOT PERFECT => The Edit part in the DatatableContentsHeadLayout doesn't appear
  .add('DatatableContents/showEdit', () => <Components.DatatableContents {...defaultProps.DatatableContents} showEdit={true} />)
  // DOES NOT WORK => PR to do on the ShowNew Props
  .add('DatatableContents/showNew', () => <Components.DatatableContents {...defaultProps.DatatableContents} showNew={true} />)
  // DOES NOT WORK
  .add('DatatableContents/empty', () => {
    return <Components.DatatableContents {...defaultProps.DatatableContents} results={[]} />;
  })
  .add('DatatableContents/selection/empty', () => (
    <Components.DatatableContents {...defaultProps.DatatableContents} currentSelection={[]} showSelect={true} results={[]} />
  ))
  .add('DatatableContents/selection', () => (
    <Components.DatatableContents
      {...defaultProps.DatatableContents}
      currentSelection={['1']}
      showSelect={true}
      toggleSelection={action('toggleSelection')}
      results={[{ _id: '1', foo: 'selected' }, { _id: '2', foo: 'not selected' }]}
    />
  ))
...
DatatableAboveStories
  ...
  .add('DatatableAbove - Interactive ', () => {
    ...
    return <Components.DatatableAbove showSearch={showSearch} showNew={showNew} canInsert={canInsert} Components={Components} />;
  });
