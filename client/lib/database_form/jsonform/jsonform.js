...
jsonform.elementTypes = {
  ...
  'text':{
    'template':'<input class="input-xlarge xlarge" type="text" ' +
  ...
  'password':{
    'template':'<input class="input-xlarge xlarge" type="password" ' +
      'name="<%= node.name %>" value="<%= escape(value) %>" id="<%= id %>"' +
      '<%= (node.disabled? " disabled" : "")%>' +
      ' />',
    'fieldtemplate': true,
  ...
  'checkboxes':{
    ...
    'onBeforeRender': function (data, node) {
      ...
      var template = '<label class="checkbox">' +
        '<input type="checkbox" <% if (value) { %> checked="checked" <% } %> name="<%= name %>" value="1"<%= (node.disabled? " disabled" : "")%>><span><%= title %></span></label>';
  ...
  'tabarray': {
    ...
    'onInsert': function (evt, node) {
      ...
      $('> a._jsonform-array-deleteitem', $nodeid).click(function (evt) {
        ...
        updateTabs();
        if (boundaries.maxItems>=0 && idx<=boundaries.maxItems-1) {
          $('> a._jsonform-array-addmore', $nodeid).removeClass("disabled");
        }
      });
      ...
      $('> a._jsonform-array-addmore', $nodeid).click(function (evt) {
        ...
        node.insertArrayItem(idx,
          $('> .tabbable > .tab-content', $nodeid).get(0));
        updateTabs(idx);
        if (boundaries.minItems > 0 && idx > boundaries.minItems - 1) {
          $('> a._jsonform-array-deleteitem', $nodeid).removeClass("disabled");
        }
      });
      ...
      if ($(node.el).sortable) {
        $('> .tabbable > .nav-tabs', $nodeid).sortable({ containment: node.el });
