...
pollVoteItem = function (collection, item, user) {
  var user = typeof user === "undefined" ? Meteor.user() : user,
      votePower = getVotePower(user),
      collectionName = collection._name.slice(0,1).toUpperCase()+collection._name.slice(1);
  ...
  // Votes & Score
  var result = collection.update({_id: item && item._id, voters: { $ne: user._id }},{
...
Meteor.methods({
  ...
  pollVote: function (post) {
    return pollVoteItem.call(this, Posts, post);
  }
});
