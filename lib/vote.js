...
//Allows getVotePower to be a Number, or a function
var parseForVoteNumber = function (user) { 
  number = Number(getVotePower);
  if(isNaN(number)){
    try{
      number = getVotePower(user);
    }catch(error){
      throw "TypeError: getVotePower is of incompatible type. It must be a Number or a Function."
    }
  }
  return number;
};
...
upvoteItem = function (collection, item, user) {
  ...
  votePower = parseForVoteNumber(user);
