  ...
  */
  ...
  canPost: function () {
    if(!this.ready() || Meteor.loggingIn()){
      ...
      toastr.error(i18n.t("sorry_you_dont_have_permissions_to_add_new_items"), "error");
      this.render(getTemplate('no_rights'));
    } else {
      this.next();
    }
  ...
  canEditPost: function() {
    ...
    if(!can.currentUserEdit(post)){
      toastr.error(i18n.t("sorry_you_cannot_edit_this_post"), "error");
      this.render(getTemplate('no_rights'));
    } else {
      this.next();
    }
  ...
  canEditComment: function() {
    ...
    if(!can.currentUserEdit(comment)){
      toastr.error(i18n.t("sorry_you_cannot_edit_this_comment"), "error");
      this.render(getTemplate('no_rights'));
    } else {
      this.next();
    }
