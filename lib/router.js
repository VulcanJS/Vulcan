...
Router._filters = {
  ...
  isLoggedIn: function(pause) {
    if (!(Meteor.loggingIn() || Meteor.user())) {
      flashMessage(i18n.t('Please Sign In First.'), "info");
  ...
  */
  ...
  canPost: function (pause) {
    if(!this.ready() || Meteor.loggingIn()){
      ...
      flashMessage(i18n.t("Sorry, you don't have permissions to add new items."), "error");
      this.render(getTemplate('no_rights'));
      pause();
    }
  },
  ...
  canEditPost: function(pause) {
    ...
    if(!currentUserCanEdit(post)){
      flashMessage(i18n.t("Sorry, you cannot edit this post."), "error");
      this.render(getTemplate('no_rights'));
      pause();
    }
  },
  ...
  canEditComment: function(pause) {
    ...
    if(!currentUserCanEdit(comment)){
      flashMessage(i18n.t("Sorry, you cannot edit this comment."), "error");
      this.render(getTemplate('no_rights'));
      pause();
    }
  },
...
if(Meteor.isClient){
  ...
  Router.onBeforeAction(clearSeenMessages);
