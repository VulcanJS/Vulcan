...
isFacebookUser=function(user){
  if(!user || typeof user === 'undefined' || typeof user.services.facebook === 'undefined')
    return false;
  return isAdmin(user) || !!user.services.facebook;
};
adminUsers = function(){
...
getUserName = function(user){
  try{
    ...
    if (user && user.services && user.services.facebook && user.services.facebook.email)
      return user.services.facebook.email.replace(/\./g,'_').replace('@','_')
  }
  catch (error){
    console.log(error);
    return null;
...
getFriendVotesUrl = function(user,friendId) {
  return getFriendVotesUrlBySlugOrId(user.slug,friendId);
};
getFriendVotesUrlBySlugOrId = function(slug,friendId) {
  return getRouteUrl('user_friend_votes', {slug: slug, friendId: friendId});
};
...
getFacebookName = function(user){
  ...
  }else if(checkNested(user, 'services', 'facebook', 'name')){
