Template[getTemplate('notificationsMenu')].helpers({
  ...
  notifications: function(){
    return Notifications.collection.find({userId: Meteor.userId(), read: false}, {sort: {timestamp: -1}});
  },
  hasNotifications: function () {
    return !!Notifications.collection.find({userId: Meteor.userId(), read: false}, {sort: {timestamp: -1}}).count();    
  },
  notification_count: function(){
    var notifications=Notifications.collection.find({userId: Meteor.userId(), read: false}).fetch();
