...
export const useUpdateOld = options => {
...
export const withUpdateOld = options => C => {
  ...
  const Wrapper = props => {
    const [updateFunc] = useUpdateOld(options);
    return <C {...props} {...{ [funcName]: updateFunc }} editMutation={legacyError} />;
  };
  Wrapper.displayName = `withUpdateOld${typeName}`;
  return Wrapper;
};

export default withUpdateOld;
