 ...
 */
export const multiQueryUpdater = ({ typeName, fragment, fragmentName, collection, resolverName }) => async (cache, { data }) => {
...
export const withCreate = options => C => {
  ...
  const Wrapper = props => {
    const [createFunc, createMeta] = useCreate(options);
    return <C {...props} {...{ [funcName]: createFunc }} {...{ [metaName]: createMeta }} newMutation={legacyError} />;
  };
