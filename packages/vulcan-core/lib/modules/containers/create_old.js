 ...
 */
export const multiQueryUpdater = ({ typeName, fragment, fragmentName, collection, resolverName }) => (cache, { data }) => {
...
export const useCreateOld = options => {
...
export const withCreateOld = options => C => {
  ...
  const Wrapper = props => {
    const [createFunc] = useCreateOld(options);
    return <C {...props} {...{ [funcName]: createFunc }} newMutation={legacyError} />;
  };

  Wrapper.displayName = `withCreateOld${typeName}`;
  return Wrapper;
};

export default withCreateOld;
