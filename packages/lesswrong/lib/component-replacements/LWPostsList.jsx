...
const LWPostsList = ({className, results, loading, count, loadMore, showHeader = true, showLoadMore = true, networkStatus, currentUser, error, terms}) => {
  ...
  if (results && results.length) {
    ...
    return (
      <div className={classNames(className, 'posts-list')}>
        ...
        {showLoadMore ?
          <Components.PostsLoadMore loading={loadingMore} loadMore={loadMore} count={count} /> :
          null
        }
      </div>
    )
...
LWPostsList.displayName = "PostsList";

LWPostsList.propTypes = {
...
replaceComponent('PostsList', LWPostsList, withCurrentUser, [withList, options]);
